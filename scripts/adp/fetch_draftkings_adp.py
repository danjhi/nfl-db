"""Fetch DraftKings Best Ball ADP via saved session → upsert into adp_sources.

Uses Playwright's own Chromium (fully independent of Chrome) with a saved
session file from setup_dk_session.py. Chrome can be open while this runs.

Setup (one-time, requires Chrome to be closed):
    python3 scripts/adp/setup_dk_session.py

Re-run setup if you see auth errors (DK session expires ~every 2 weeks).

Usage:
    python3 scripts/adp/fetch_draftkings_adp.py [--dry-run]
"""
...
# DK API position IDs → standard position strings
POSITION_MAP = {1: "QB", 2: "RB", 3: "WR", 4: "TE", 5: "K", 6: "DST"}


def fetch_dk_players():
    """Use saved session + Playwright Chromium to fetch the DK playerpool JSON."""
    try:
    ...
    with sync_playwright() as p:
        ...
        print(f"  Fetching DK playerpool API...")
...
def main():
    ...
    # ── 1. Fetch DK data via Playwright ──────────────────────────────────────
    print("Fetching DraftKings ADP...")
    ...
    for player in dk_players:
        ...
        # Get position from roster slot mapping
        pos = ""
