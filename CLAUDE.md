...
| `setup_dk_session.py` | One-time setup: extract DK cookies from Chrome via `browser_cookie3` → save to `data/dk_session.json`. Profile comes from `DK_CHROME_PROFILE` in `.env` (code default `Profile 2`; **laptop .env sets `Default`** — the danhindery@gmail.com profile, the only one with a DK login there). Re-run when daily fetch returns auth errors (~2 weeks). Chrome must be logged in to DK in that profile; Chrome can stay open. |
| `setup_underdog_session.py` | One-time setup: extract Underdog cookies (both `underdogfantasy.com` and `underdogsports.com`) from Chrome → save to `data/underdog_session.json`. Profile comes from `UD_CHROME_PROFILE` in `.env` (code default `Profile 2`; **laptop .env sets `Default`**). Re-run when daily fetch returns Cloudflare 403 (~every 2 weeks). Required after Underdog domain migration + Cloudflare bot management (May 2026). |
...
| `fetch_yahoo_adp.py` | Fetch Yahoo redraft ADP via the **public read-only** fantasy API (`pub-api-ro.fantasysports.yahoo.com/fantasy/v2/game/nfl/players;sort=AR;out=draft_analysis;count=300?format=json_f`) — **no OAuth/login**. ADP at `draft_analysis.average_pick`; `player_id` **is** `players.yahoo_id`. Join: offense/K by `yahoo_id` → name+pos fallback; D/ST (`display_position="DEF"`) remaps → `DEF_{TEAM}` via `editorial_team_abbr`. ~216 rows. `--dry-run`. |
| `fetch_cbs_adp.py` | Fetch CBS redraft ADP (static HTML at `cbssports.com/fantasy/football/draft/averages`, no login) → upsert `source="cbs"`. Three pages: offense PPR (`/ppr/both/h2h/all/`), kickers (`/both/h2h/K/`), D/ST (`/both/h2h/DST/`) — K/DST aren't PPR so they're on the non-PPR path. Each row's href carries a `cbs_id` → `players.cbs_id`; name+pos fallback; D/ST remaps → `DEF_{TEAM}` by team. ADP = 4th table cell. Regex-parsed on CBS's `TableBase`/`CellPlayerName` classes — re-check if they restyle. `--dry-run`. |
| `fetch_espn_adp.py` | Fetch ESPN redraft ADP (`lm-api-reads.fantasy.espn.com/.../leaguedefaults/3?view=kona_player_info`, no login) → upsert `source="espn"`. ADP at `player.ownership.averageDraftPosition`; one request via `x-fantasy-filter` `sortPercOwned` (ESPN rejects a `limit` without a sort). Auto-detects & drops ESPN's undrafted **sentinel cluster** (~793 players pinned at ~170) so we store just the drafted board (~206). Join: offense/K by `players.espn_id` → name+pos fallback; D/ST (pos 16, id `-(16000+proTeamId)`) remaps → `DEF_{TEAM}` via `ESPN_TEAM` proTeamId map. `--dry-run`. |
| `fetch_nffc_adp.py` | Fetch NFFC ADP (`nfc.shgn.com/api/public/adp/football`, `NFFC_API_KEY`) → upsert three sources: `nffc_oc` (`game_type_id=936`, the $350 FBG Online Championship — displayed), `nffc` (all contests, no game_type_id — the old "NFFC" catch-all, kept as a hedge), and `bestball10s` (`game_type_id=941`, NFFC BestBall10s — best-ball tab, replaces FBG's `bestball10s` key). NFFC's `player` id **is** `players.player_id` (Sportradar UUID) so offense joins on the id directly (immune to the Marquise/AJ Brown name-swap in FBG's stored data). NFFC drafts **team** K/DST: `TDSP` remaps → `DEF_{TEAM}` by team (converges with other sources); `TK` stays on its own team-kicker row (NFFC int id == `players.player_id`, no `footballguys_id` so it's NFFC-only and excluded from the individual-kicker consensus). `game_type_id=936` is 2026-specific — bump yearly (see `scripts/nffc/build_oc_local_fixture.py`). `--dry-run`, `--source`. Set UA header (403 otherwise). |
...
| `fetch_draftkings_lines.py` | DK-only full-season scraper via Playwright. Drives a **headful** Chromium (since 2026-07: DK's "stadium" redesign serves an empty shell to headless — client hints leak `HeadlessChrome`; a window pops up ~30s/run) against `sportsbook.draftkings.com/leagues/football/nfl?category=games&subcategory=game-lines&nav_1=all` (query params required — the bare league URL no longer lands on the game-lines board; API subcategory id now `4518`, was `4`), clicks the "View More" button (`.cms-market-selector-load-more-button`) to paginate, captures `sportscontent/controldata/.../markets` XHR responses (first click re-fetches from page 1, so the SSR-rendered initial batch isn't missed). Extracts spread/total/moneyline from the `selections.points` + `displayOdds.american` fields. **Full 272-game coverage available here even when The Odds API only shows the next 5-6 weeks.** Bookmaker tag: `draftkings`. Requires Playwright + Chromium installed. Run with `SSL_CERT_FILE=$(python3 -m certifi)` if SSL fails. `--headless` flag kept for retesting (captures 0 events as of 2026-07-16). **Partial boards are normal, not an error** (policy 2026-07-16): in-season DK trims to upcoming weeks; the script upserts whatever is posted and logs `Board coverage: N/M`. Absent games keep their most recent prior snapshot (PK includes date) and all consumers (bestballbesty/fbg-best-ball schedule tabs, ff-stat-sim implied-totals parquet) read latest-available per game — worst case the 2026-05-15 openers. Only a zero-event capture exits nonzero. |
...
Chrome cookies (profile per `DK_CHROME_PROFILE` in .env) → setup_dk_session.py → data/dk_session.json → fetch_draftkings_adp.py (Playwright) → Supabase adp_sources table (daily snapshots, source="draftkings")

Chrome cookies (profile per `UD_CHROME_PROFILE` in .env) → setup_underdog_session.py → data/underdog_session.json → fetch_underdog_postdraft_adp.py (Playwright + Cloudflare-passing stealth args) → Supabase adp_sources (source="underdog_postdraft", new players.underdog_postdraft_id column for ID matching)
...
| NFFC ADP (laptop-primary, Jul 10–Sep 10) | `~/Library/LaunchAgents/com.nfldb.laptop-nffc-adp.plist` | 1:20 PM | venv Python → `fetch_nffc_adp.py` (`source=nffc_oc` + `nffc` + `bestball10s`). Version-controlled in `scripts/launchd/laptop/`. Log `data/logs/nffc_adp.log`. `game_type_id` 936/941 are 2026-specific — bump yearly. |
...
| `bestball10s` | 2026 | daily (via `fetch_nffc_adp.py`) | NFFC BestBall10s (`game_type_id=941`). ~254 rows/snapshot. Best-ball-tab source; replaces the `bestball10s` key FBG's feed carries (value-matched). |
| `espn` | 2026 | daily (laptop 1:25 PM) | ESPN redraft (`kona_player_info`, PPR leaguedefaults/3). ~206 rows/snapshot (drafted board after dropping ESPN's ~170 undrafted sentinel). Replaces FBG feed's `espn`. |
| `cbs` | 2026 | daily (laptop 1:30 PM) | CBS redraft (static HTML, offense PPR + K + DST). ~215 rows/snapshot. Matches on `cbs_id`. Replaces FBG feed's `cbs`. |
| `yahoo` | 2026 | daily (laptop 1:35 PM) | Yahoo redraft (public read-only fantasy API, no auth). ~216 rows/snapshot. Matches on `yahoo_id`. Replaces FBG feed's `yahoo`. |
...
- DK sportsbook pages are behind Akamai bot mgmt (`sportsbook-nash.draftkings.com` returns 403 to plain urllib). Must drive Chromium so `_abck` / `bm_sz` cookies attach — and since the 2026-07 "stadium" redesign it must be **headful** (headless gets an empty shell: no odds section, no View More button, 0 XHRs; the initial board arrives via SSR + msgpack WebSocket instead of XHR). Pagination via clicking `.cms-market-selector-load-more-button` (100 events per page; 2-3 clicks gets all 272, and the first click re-fetches page 1 via XHR)
