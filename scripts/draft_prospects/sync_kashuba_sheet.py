...
def fetch_table() -> list[dict]:
    url = (f"{SUPABASE_URL}/rest/v1/{TABLE}?draft_year=eq.{DRAFT_YEAR}"
           "&select=dan_id,name,position,college,value,sf_value,values_updated_at,updated_at"
           "&order=dan_id&limit=1000")
...
def main() -> int:
    ...
    if args.apply and changes:
        ...
        for c in changes:
            dan_id = c["t"]["dan_id"]
            payload = {"value": c["value"], "sf_value": c["sf_value"],
                       "values_updated_at": now_iso, "updated_at": now_iso}
            try:
        ...
        print(f"applied: {applied}/{len(changes)}" + (f"  FAILED: {len(failures)}" if failures else ""))
        for f in failures:
            print(f"  - {f}")
        if applied:
...
def restore(backup: Path) -> int:
    ...
    current = {t["dan_id"]: t for t in fetch_table()}
    now_iso = dt.datetime.now(dt.timezone.utc).isoformat()
    restored = skipped = 0
    failures: list[str] = []
    ...
    for r in rows:
        ...
        if cur is None:
            ...
            continue
        if num(cur["value"]) == num(r["value"]) and num(cur["sf_value"]) == num(r["sf_value"]):
            skipped += 1
            continue
...
def write_status(sh, started, applied_mode, n_sheet, n_matched, unchanged, changes, applied,
                 failures, held, problems, unmatched, missing_from_sheet) -> None:
    import gspread
    ...
    lines = [
        ...
        ["", ""],
        ["How values reach the site", "value / SF_Value are copied into DTVC Plus daily; the site refreshes within ~4 hours. "
